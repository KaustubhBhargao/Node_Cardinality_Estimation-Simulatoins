...
T = 2; % types of nodes
% T = T_mat(T_ind);
D = 1000; % total nodes per type, starting with twice as many because area is twices as before
...
%     fprintf('no. of active nodes for q = %d is: %d\n',q, sum(sum(active)));
    ...
    for m=1:M % for every stop, execute both phase 1 and phase two
        ...
        %stop
        %fprintf('at stop = %d, x = %f, y = %f\n', m, mbs_stops(m,1), mbs_stops(m,2));
