...
ncc = 3; % (alpha,C):{1,2}:(C,beta):{3,4);(C,C):{1,2,3,4}
b = 6; % slot width
...
% var_D = 210; D_mat = [580, 790, 1210, 1420];
% var_q = 0.009624; q_mat = [0.280752, 0.290376, 0.309624, 0.319248];
var_D = 132; D_mat = [736, 868, 1132, 1264];
var_q = 0.029991; q_mat = [0.240018, 0.270009, 0.329991, 0.359982];
...
%                 temp(h,b,m) = temp(h,b,m) + active_inrange(b,i2)*random('bino',1,2^(-I(b,m)/l));
        ...
        for ind=1:l
            switch 1
               case ((temp(ind,1,m) == 0) && (temp(ind,2,m)==1) && (temp(ind,3,m)>=1) && (temp(ind,4,m)==0)) % (alpha,C) case
                    R1=R1+1;
               case ((temp(ind,1,m) == 1) && (temp(ind,2,m)==0) && (temp(ind,3,m)>=2) && (temp(ind,4,m)==0)) % (alpha,C) case
                    R1=R1+1;
               case ((temp(ind,3,m) == 0) && (temp(ind,4,m)==1) && (temp(ind,1,m)>=1) && (temp(ind,2,m)==0)) % (C, beta) case
                    R1=R1+1;
               case ((temp(ind,3,m) == 1) && (temp(ind,4,m)==0) && (temp(ind,1,m)>=2) && (temp(ind,2,m)==0)) % (C, beta) case
                    R1=R1+1;
               ...
               case ((temp(ind,2,m) == 1) && (temp(ind,4,m)==1)) % (C,C) case
                    R2=R2+1;
               case ((temp(ind,1,m) >= 1) && (temp(ind,2,m)==1) && (temp(ind,3,m)>=1) && (temp(ind,4,m)==0)) % (C,C) case
                    R2=R2+1;
               case ((temp(ind,1,m) >= 1) && (temp(ind,2,m)==0) && (temp(ind,3,m)>=1) && (temp(ind,4,m)==1)) % (C,C) case
                    R2=R2+1;
               case ((temp(ind,1,m) >= 2) && (temp(ind,2,m)==0) && (temp(ind,3,m)>=2) && (temp(ind,4,m)==0)) % (C,C) case
                    R2=R2+1;
            end
...
%         Prop_Method(m) = Prop_Method(m) + (T-1)*l+ (len*l)/b + R1 + R2*(M2T2/l)*2; % with BP lenght
        Prop_Method(m) = Prop_Method(m) + 2*l+ 1 + R1+ R2*(M2T2/l)*2; % without BP lenght, factor of 2 because two seperate groups        
%         Prop_Method(m) = Prop_Method(m) + 2*l+ 1 + R1+ R2*(M2T2_vs_eps(eps_ind)/l)*2;
    end % end loop for m
...
save M2T4.txt avg_slots -ascii %To save the values, which can be used in 4dev, 6dev etc.
% save M2T4_vs_D.txt Prop_Method_D -ascii 
% save M2T4_vs_eps.txt Prop_Method_eps -ascii
% save M2T4_vs_q.txt Prop_Method_q -ascii 