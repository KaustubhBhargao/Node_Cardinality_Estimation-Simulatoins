...
ncc = 1; % (C,C) case
b = 6; %slot width considered
len = ceil(log2(ncc)); %lenght of broadcast packet
...
% D_mat = [869, 1000, 1131];
% var_q = 0.158132; 
% q_mat = [0.141868, 0.300000, 0.458132];
var_D = 185; D_mat = [815, 1000, 1185];
var_q = 0.083053; q_mat = [0.216947, 0.300000, 0.383053];
...
%                 temp(h,b,m) = temp(h,b,m) + active_inrange(b,i2)*random('bino',1,2^(-I(b,m)/l));
        ...
        for ind=1:l
            switch 1
%                case ( (temp(ind,1,m) == 0) && (temp(ind,2,m)==1) && (temp(ind,3,m)>=1)) % (alpha,C) case
%                     R1=R1+1;
%                case ( (temp(ind,1,m) == 1) && (temp(ind,2,m)==0) && (temp(ind,3,m)>=2)) % (alpha,C) case
%                     R1=R1+1;
%                case ((temp(ind,2,m)>=2)) % (C,C) case
%                     R2=R2+1;
%                case ( (temp(ind,1,m) >= 1) && (temp(ind,2,m)==1) && (temp(ind,3,m)>=1)) % (C,C) case
%                     R2=R2+1;
%                case ( (temp(ind,1,m) >= 2) && (temp(ind,2,m)==0) && (temp(ind,3,m)>=2)) % (C,C) case
%                     R2=R2+1;
               case ((temp(ind,3,m)>=2)) % (C,C) case 
                    % when multiple nodes of only Type 3 transmit
                    R2=R2+1;
               case ((temp(ind,1,m)>=1) && (temp(ind,2,m)>=1) && (temp(ind,3,m)==0)) % (C,C) case
                    % when multiple nodes of both Type 1 and 2 tx, Type 3
                    % doesn't
                    R2=R2+1;
%                case ((temp(ind,1,m)>=1) && (temp(ind,2,m)==0) && (temp(ind,3,m)>=2)) % (C,C) case
%                     % when only Type 1 and 3 tx
%                     R2=R2+1;
%                case ((temp(ind,1,m)==0) && (temp(ind,2,m)>=1) && (temp(ind,3,m)>=2)) % (C,C) case
%                     % when only type 2 and 3 tx
%                     R2=R2+1;
            end
...
%         Prop_Method(m) = Prop_Method(m) + (T-1)*l+ (len*l)/b + R1 + R2*(M2T2/l); % with BP lenght
        Prop_Method(m) = Prop_Method(m) + 2*l+ 1 + R1+ R2*(M2T2/l); % without BP lenght
%         Prop_Method(m) = Prop_Method(m) + 2*l+ 1 + R1+ R2*(M2T2_vs_eps(eps_ind)/l);
    end % end loop for m
...
save M2T3.txt avg_slots -ascii %To save the values, which can be used in 4dev, 6dev etc.
% save M2T3_vs_D.txt Prop_Method_D -ascii
% save M2T3_vs_eps.txt Prop_Method_eps -ascii
% save M2T3_vs_q.txt Prop_Method_q -ascii