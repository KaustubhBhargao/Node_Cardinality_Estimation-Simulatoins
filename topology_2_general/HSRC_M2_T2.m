...
% var_D = 293; 
% D_mat = [707, 1293];
% var_q = 0.106935; q_mat = [0.193065, 0.406935];
var_D = 359; D_mat = [641, 1359];
var_q = 0.057686; q_mat = [0.242314, 0.357686];
...
%                 temp(h,b,m) = temp(h,b,m) + active_inrange(b,i2)*random('bino',1,2^(-I(b,m)/l));
        ...
        R=0;
        for ind=1:l
           if((temp(ind,1,m)>=2)|| (temp(ind,2,m)>=2) || ((temp(ind,1,m)== 1)&&(temp(ind,2,m)== 1))) %Specific to 2 types of nodes case.
                    K=K+1;          
           end
           if ((temp(ind,1,m)>=2))
                R=R+1;
        ...
        if (K == 0)
            Prop_Method(m) = Prop_Method(m) + (T-1)*l + 1; %First phase 2t slots, 1 for BP after 1st phase by BS, K second phase, 1 for BP after 2nd phase(it may be zero). R third Phase
        else
            Prop_Method(m) = Prop_Method(m) + (T-1)*l + 1 + K + 1+ (T-1)*R; %First phase 2t slots, 1 for BP after 1st phase by BS, K second phase, 1 for BP after 2nd phase(it may be zero). R third Phase
        end
    end % end loop for m
...
save M2T2.txt avg_slots -ascii %To save the values, which can be used in 4dev, 6dev etc.
% save M2T2_vs_D.txt Prop_Method_D -ascii 
% save M2T2_vs_eps.txt Prop_Method_eps -ascii 
% save M2T2_vs_q.txt Prop_Method_q -ascii 